...
def a_star_exp3_aux(source, target, abstractMap, experiment=False):
    ...
    try:
        ...
        for center in list(abstractMap.keys()):
            ...
            current_distance = tools.compute_distance(Roads[center].lat,Roads[center].lon,Roads[source].lat,Roads[source].lon)
        ...
        (a_star_path,a_cost,develped_a) = astar(Roads, source, junc1_index, h_func=lambda junc: h_func_aux(junc, Roads[junc1_index]))
        cost += a_cost
        if a_star_path:
        ...
        #run ucs from junc2 to B to get the path
        (a_star_path,b_cost,develped_b) = astar(Roads, junc2_index, target, h_func=lambda junc: h_func_aux(junc, Roads[target]))
        cost += b_cost
        if a_star_path:
        ...
        (a_star_path,c_cost,develped_c) = astar(abstractMap, junc1_index, junc2_index, h_func=lambda junc: h_func_aux(junc, Roads[junc2_index]),cost_func=lambda x: x.cost)
        cost += c_cost
        if a_star_path:
            #path for j1 to j2
    ...
    # exception is caught therefor we need to run norma ucs from A to B
        ...
        (a_star_path, cost, fail_num_dev)  = astar(Roads, source, target, h_func = lambda junc : h_func_aux(junc,Roads[target]))
        if experiment:
